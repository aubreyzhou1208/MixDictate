...
struct Config {
    ...
    /// **默认 0，也就是关闭。**
    ///
    /// 门限有个绕不过去的漏洞：它只能按响度判断，而"整句都说得轻"和
    /// "这是外放漏进来的"在响度上就是同一件事。「呃，这个东西，它能跑通吗？」
    /// 这种一路都轻的说法，门限可能从头到尾没开过 —— 那一整句就没了。
    ///
    /// 为它做过的补救（回滞、开口前多压一秒、尾部一律放行）都只在门限
    /// **最终会打开**时才有用。整句够不着门限的话，一样救不回来。
    ///
    /// 所以默认关掉。它是给"必须外放着听写"那种场景准备的开关，不是常备件。
    /// 真要完全隔绝外放，戴耳机才是唯一可靠的办法。
    ///
    /// 打开的话：调大更能挡外放，调小小声说话也收得到。
    var voiceThreshold: Double = 0
    ...
    ///
    /// 从 45 提到 120：完整重转不光更准，还**不可能丢段落** —— 拼接那条路
    /// 要靠一串中间结果都正确累积起来，环节多得多。多等几秒远比丢一整段好。
    var fullPassMaxSeconds: Double = 120
