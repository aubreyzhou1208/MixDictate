...
代价是同一段音频会被转很多遍，而且**录得越久单次开销越大**。固定 1.2 秒
间隔的话，说 10 秒累计要转掉约 45 秒音频 —— 是最终那次的 4.5 倍，GPU 全被
中间请求占着，松手后的结果反而要排队。

所以间隔跟着已录时长缓慢增长并封顶：`min(2.5, max(基础间隔, 已录时长 × 0.25))`。
（第一版写成 `× 0.5` 且不封顶，说到 10 秒要等 5 秒才刷新一次，用起来像卡住了。）

另一个省时间的地方：松手时如果音频比最后一次中间结果几乎没长（0.4 秒以内），
那次的文字就直接当最终结果用，跳过整整一次推理 —— 松手后的等待正是整个流程
里最难受的一段。
